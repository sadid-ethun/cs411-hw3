...
def test_get_leaderboard(mock_cursor):
    """Test retrieving the leaderboard of meals that are not marked as deleted."""

    # Simulate that there are multiple meals in the database
    mock_cursor.fetchall.return_value = [
        (2, "Sushi", "Japanese", 15.99, "HIGH", 8, 6, 0.75),
        (3, "Burger", "American", 8.99, "LOW", 10, 4, 0.4),
        (1, "Pizza", "Italian", 10.99, "MED", 5, 3, 0.6)
    ]

    # Call the get_leaderboard function, assuming default sorting by "wins"
    leaderboard = get_leaderboard(sort_by="wins")

    # Ensure the results match the expected output ordered by wins
    expected_result = [
        {'id': 2, 'meal': 'Sushi', 'cuisine': 'Japanese', 'price': 15.99, 'difficulty': 'HIGH', 'battles': 8, 'wins': 6, 'win_pct': 75.0},
        {'id': 3, 'meal': 'Burger', 'cuisine': 'American', 'price': 8.99, 'difficulty': 'LOW', 'battles': 10, 'wins': 4, 'win_pct': 40.0},
...
def test_get_leaderboard_win_pct(mock_cursor):
    ...
    # Call the get_leaderboard function with sorting by "win_pct"
    leaderboard = get_leaderboard(sort_by="win_pct")

    # Ensure the results match the expected output ordered by win percentage
    expected_result = [
        {'id': 2, 'meal': 'Sushi', 'cuisine': 'Japanese', 'price': 15.99, 'difficulty': 'HIGH', 'battles': 8, 'wins': 6, 'win_pct': 75.0},
        {'id': 1, 'meal': 'Pizza', 'cuisine': 'Italian', 'price': 10.99, 'difficulty': 'MED', 'battles': 5, 'wins': 3, 'win_pct': 60.0},
        {'id': 3, 'meal': 'Burger', 'cuisine': 'American', 'price': 8.99, 'difficulty': 'LOW', 'battles': 10, 'wins': 4, 'win_pct': 40.0}
...
def test_get_meal_by_name(mock_cursor):
    # Simulate that the meal exists (name = "Pizza", cuisine = "Italian", price = 10.99, difficulty = "MED")
    mock_cursor.fetchone.return_value = (1, "Pizza", "Italian", 10.99, "MED", False)
...
def test_update_meal_stats_win(mock_cursor):
    ...
    # Normalize the expected SQL query for a win update
    expected_query = normalize_whitespace("""
...
def test_update_meal_stats_loss(mock_cursor):
    ...
    # Normalize the expected SQL query for a loss update
    expected_query = normalize_whitespace("""
...
def test_update_meal_stats_deleted_meal(mock_cursor):
    ...
    # Ensure that no SQL query for updating stats was executed beyond the check
...
def test_update_meal_stats_not_found(mock_cursor):
    ...
    # Ensure that only the SELECT query was executed

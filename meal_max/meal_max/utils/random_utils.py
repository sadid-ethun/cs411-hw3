...
def get_random() -> float:
    """Gets a random decimal number from random.org between 0 and 1.

    This function makes a request to the random.org API to get a
    random decimal number with two decimal places. The number is used
    in several applications where a random float is necessary.

    Returns:
        A random float number retrieved from random.org.
